...
public class CircleEvent extends EditAnnotEvent {
    public CircleEvent(int eventType, CircleUndoItem undoItem, Circle circle, PDFViewCtrl pdfViewCtrl) {
        mType = eventType;
        mUndoItem = undoItem;
        mAnnot = circle;
    ...
    @Override
    public boolean add() {
        if (mAnnot == null || !(mAnnot instanceof Circle)) {
            return false;
        }
        Circle annot = (Circle) mAnnot;
        try {
