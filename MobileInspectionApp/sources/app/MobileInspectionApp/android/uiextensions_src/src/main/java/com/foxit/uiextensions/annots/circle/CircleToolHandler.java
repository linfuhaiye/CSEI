...
public class CircleToolHandler implements ToolHandler {
    ...
    /**
     * one of mCreateList to draw the CIR
     */
     ...
     */
    protected void init() {
        mColor = PropertyBar.PB_COLORS_CIRCLE[6];
        mOpacity = 100;
        mThickness = 5.0f;
        ...
        mUiExtensionsManager.getMainFrame().getMoreToolsBar().registerListener(new MoreTools.IMT_MoreClickListener() {
            ...
            public void onMTClick(int type) {
                mUiExtensionsManager.setCurrentToolHandler(CircleToolHandler.this);
    ...
    private void resetAnnotBar(){
        ...
        mPropertyItem = new PropertyCircleItemImp(mContext) {
            ...
            public void onItemLayout(int l, int t, int r, int b) {
                if (CircleToolHandler.this == mUiExtensionsManager.getCurrentToolHandler()) {
    ...
    public void onDraw(int pageIndex, Canvas canvas) {
        ...
        if (mLastPageIndex == pageIndex) {
            canvas.save();
            setPaint(pageIndex);
            canvas.drawArc(mNowRect, 0, 360, false, mPaint);
            canvas.restore();
        }
    }
    ...
    private void createAnnot() {
        if (mPdfViewCtrl.isPageVisible(mLastPageIndex)) {
            ...
            try {
                ...
                final Circle newAnnot = (Circle) AppAnnotUtil.createAnnot(page.addAnnot(Annot.e_Circle, AppUtil.toFxRectF(bboxRect)), Annot.e_Circle);
                ...
                CircleEvent event = new CircleEvent(EditAnnotEvent.EVENTTYPE_ADD, undoItem, newAnnot, mPdfViewCtrl);
