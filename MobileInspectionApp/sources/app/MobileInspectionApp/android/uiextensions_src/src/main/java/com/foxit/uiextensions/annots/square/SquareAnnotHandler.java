...
public class SquareAnnotHandler implements AnnotHandler {
    ...
    public void onColorValueChanged(int color) {
        ...
        try {
            if (annot != null && uiExtensionsManager.getCurrentAnnotHandler() == this && color != annot.getBorderColor()) {
                ...
                modifyAnnot(annot.getPage().getIndex(), annot, rectF, color, (int) (((Square) annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(), false, false, null);
            }
    ...
    public void onOpacityValueChanged(int opacity) {
        ...
        UIExtensionsManager uiExtensionsManager = (UIExtensionsManager) mPdfViewCtrl.getUIExtensionsManager();
        try {
            if (annot != null && uiExtensionsManager.getCurrentAnnotHandler() == this && opacity != (int) (((Square) annot).getOpacity() * 255f)) {
                RectF rectF = AppUtil.toRectF(annot.getRect());
                modifyAnnot(annot.getPage().getIndex(), annot, rectF, (int) annot.getBorderColor(), AppDmUtil.opacity100To255(opacity), annot.getBorderInfo().getWidth(), annot.getContent(), false, false, null);
            }
        } catch (PDFException e) {
            e.printStackTrace();
        }
    ...
    public void onLineWidthValueChanged(float lineWidth) {
        ...
        try {
            if (annot != null && uiExtensionsManager.getCurrentAnnotHandler() == this && lineWidth != annot.getBorderInfo().getWidth()) {
                ...
                modifyAnnot(annot.getPage().getIndex(), annot, bboxRect, (int) annot.getBorderColor(), (int) (((Square) annot).getOpacity() * 255f), lineWidth, annot.getContent(), false, false, null);
    ...
    private void modifyAnnot(final int pageIndex, final Annot annot, RectF bbox, int color, int opacity, float lineWidth, String contents, boolean isModifyJni, final boolean addUndo, final Event.Callback result) {// to be continued
        final SquareModifyUndoItem undoItem = new SquareModifyUndoItem(mPdfViewCtrl);
        ...
        if (isModifyJni) {
            ...
            SquareEvent event = new SquareEvent(EditAnnotEvent.EVENTTYPE_MODIFY, undoItem, (Square) annot, mPdfViewCtrl);
    ...
    private void deleteAnnot(final Annot annot, final boolean addUndo, final Event.Callback result) {
        ...
        try {
            ...
            SquareEvent event = new SquareEvent(EditAnnotEvent.EVENTTYPE_DELETE, undoItem, (Square) annot, mPdfViewCtrl);
    ...
    public void onAnnotDeselected(Annot annot, boolean needInvalid) {
        ...
        try {
            ...
            if (needInvalid && mIsModify) {
                ...
                RectF rectF = AppUtil.toRectF(annot.getRect());
                if (mTempLastColor == (int) annot.getBorderColor() && mTempLastLineWidth == annot.getBorderInfo().getWidth() && mTempLastBBox.equals(AppUtil.toRectF(annot.getRect())) && mTempLastOpacity == (int) (((Square) annot).getOpacity() * 255f)) {
                    modifyAnnot(page.getIndex(), annot, rectF, (int) annot.getBorderColor(), (int) (((Square) annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(), false, false, null);
                } else {
                    modifyAnnot(page.getIndex(), annot, rectF, (int) annot.getBorderColor(), (int) (((Square) annot).getOpacity() * 255f), annot.getBorderInfo().getWidth(), annot.getContent(), true, true, null);
                }
            } else if (mIsModify) {
    ...
    private void preparePropertyBar(boolean canEdit) {
        mAnnotationProperty.setEditable(canEdit);
        ...
        try {
            ...
            int opacity = AppDmUtil.opacity255To100((int) (((Square) ((UIExtensionsManager)mPdfViewCtrl.getUIExtensionsManager()).getDocumentManager().getCurrentAnnot()).getOpacity() * 255f + 0.5f));
    ...
    public void addAnnot(final int pageIndex, AnnotContent contentSupplier, final boolean addUndo, final Event.Callback result) {
        ...
        try {
            ...
            final Annot annot = AppAnnotUtil.createAnnot(page.addAnnot(Annot.e_Square, AppUtil.toFxRectF(contentSupplier.getBBox())), Annot.e_Square);
            ...
            SquareEvent event = new SquareEvent(EditAnnotEvent.EVENTTYPE_ADD, undoItem, (Square) annot, mPdfViewCtrl);
            EditAnnotTask task = new EditAnnotTask(event, new Event.Callback() {
                @Override
                public void result(Event event, boolean success) {
                    if (success) {
                ...
                case MotionEvent.ACTION_CANCEL:
                    ...
                    if (mTouchCaptured && annot == ((UIExtensionsManager)mPdfViewCtrl.getUIExtensionsManager()).getDocumentManager().getCurrentAnnot() && pageIndex == annot.getPage().getIndex()) {
                        ...
                        if (mLastOper != OPER_DEFAULT && !mDownPoint.equals(mLastPoint.x, mLastPoint.y)) {
                            ...
                            modifyAnnot(pageIndex, annot, bboxRect, (int) annot.getBorderColor(), (int) (((Square) annot).getOpacity() * 255f), _lineWidth, annot.getContent(), false, false, null);
    ...
    public void onDraw(int pageIndex, Canvas canvas) {
        ...
        try {
            ...
            if (AppAnnotUtil.equals(mBitmapAnnot, annot) && annotPageIndex == pageIndex) {
                ...
                float thickness = thicknessOnPageView(pageIndex, annot.getBorderInfo().getWidth());
                mPathPaint.setColor(annot.getBorderColor() | 0xFF000000);
