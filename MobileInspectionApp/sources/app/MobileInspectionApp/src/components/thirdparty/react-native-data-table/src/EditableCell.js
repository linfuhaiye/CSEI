...
export class EditableCell extends React.Component {
    static getDerivedStateFromProps(nextProps, prevState) {
        return nextProps.value !== prevState.value ? { value: String(nextProps.value) } : null;
    }
    ...
    render() {
        ...
        return (
            <View style={[defaultStyles.cell, style, { flex: width }]}>
                <TextInput {...textInputProps} ref={refCallback} style={[defaultStyles.text, textStyle]} onChangeText={(text) => this.setState({ value: text })} onEndEditing={this.onEndEditing} value={this.state.value} />
            </View>
        );
    }
}
